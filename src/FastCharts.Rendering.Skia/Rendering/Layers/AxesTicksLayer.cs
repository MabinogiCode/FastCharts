        ...
        public void Render(RenderContext ctx)
        {
            ...
            foreach (var t in xTicks)
            {
                ...
                if (xIsCategory)
                {
                    ...
                    var categoryAxis = (CategoryAxis)model.XAxis;
                    var categoryLabel = categoryAxis.GetCategoryAt(t);
                    lbl = categoryLabel ?? t.ToString("F0", CultureInfo.InvariantCulture);
                }
                else if (xIsDate)
                {
            ...
            foreach (var t in yTicks)
            {
                ...
                {
                    var categoryLabel = categoryYAxis.GetCategoryAt(t);
                    lbl = categoryLabel ?? t.ToString("F0", CultureInfo.InvariantCulture);
                }
                else
                {
            ...
            if (y2Ticks != null)
            {
                ...
                foreach (var t in y2Ticks)
                {
                    ...
                    {
                        var categoryLabel = categoryY2Axis.GetCategoryAt(t);
                        lbl = categoryLabel ?? t.ToString("F0", CultureInfo.InvariantCulture);
                    }
                    else
                    {
